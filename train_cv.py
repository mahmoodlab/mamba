...
def train(conf,
    ...
    for epoch in tqdm(range(conf['epochs'])):
        ...
        #     if train_metrics:
        #         if conf['task'] == 'clf':
        #             writer.add_scalar('test/bal_acc', test_metrics['bal_acc'], global_step=epoch)
        #             writer.add_scalar('test/auc', test_metrics['auc'], global_step=epoch)
        #             writer.add_scalar('test/f1',  test_metrics['f1'], global_step=epoch)
...
def process_results(conf, results_fold, evaler_all, data_lists, result_path):
    ...
    if conf['task'] == 'clf':
        ...
        # time_bins = get_time_bins(df, numOfbins=(conf['numOfclasses'] - 1))
        # print(result_path)
        # print("Compute IBS...")
        # # Compute integrated Brier score
        # ibs = estimate_ibs(event_status=data_lists['event'],
        #                 surv_times=data_lists['survival'],
        #                 surv_func=surv_func,
        #                 time_bins=time_bins)
        # eval_results['ibs'] = ibs
        #
        # results = {'pred_risk': pred_risk,
        #         'event': data_lists['event'],
        #         'survival': data_lists['survival'],
        #         'indices': data_lists['indices'],
        #         'metrics': eval_results,
        #         'epoch': data_lists['epoch']
        #         }
        #
        # with open(os.path.join(result_path, 'result.pkl'), 'wb') as f:
        #     pickle.dump(results, f)
        #
        # # Plot KM curve
        # class_list = classify_surv(pred_risk, numOfclasses=2, class_labels=['high', 'low'])
        #
        # chisq, pvalue, stats, covar = compare_survival(data_surv, class_list, return_stats=True)
        #
        # surv_dict, event_dict = prepare_surv_dict(data_lists['survival'], data_lists['event'], class_list)
        # title = 'p-val: {0:.3e} c-index: {1:.3f}'.format(pvalue, eval_results['c_index'])
        # print(title)
        # plot_KM(surv_dict,
        #         event_dict,
        #         title=title,
